...
class INS {
  ...
  public:
    ...
    /**
     * Return the gravity due to the Earth's rotation as
     * @f[
     *    \tilde{q}_{e}^{n} {}^{\ast}
     *      \left\{ \begin{array}{c} 0 \\
     *        \vec{\omega}_{e/i}^{e} \times \left( \vec{\omega}_{e/i}^{e} \times \vec{r}_{e} \right)
     *        \end{array} \right\}
     *      \tilde{q}_{e}^{n} \quad .
     * @f]
     *
     * @return (vec3_t) gravity generated by the Earth's rotation in the navigation frame
     */
    vec3_t gravity_rotational() const {
      vec3_t centripetal_f(
            q_e2n[1] * q_e2n[3] + q_e2n[0] * q_e2n[2], // -cos(lambda) * cos(phi) / 2
            q_e2n[3] * q_e2n[2] - q_e2n[1] * q_e2n[0], // -sin(lambda) * cos(phi) / 2
            0);
      centripetal_f *= (pow2(Earth::Omega_Earth) * (Earth::R_normal(phi) + h) * 2);
      return (q_e2n.conj() * centripetal_f * q_e2n).vector();
    }
    ...
    virtual vec3_t gravity_total() const {
      vec3_t res(0, 0, Earth::gravity(phi, h));

      if(false){
        /* WGS84 theoretical normal gravity on and above the ellipsoidal surface
         * includes contribution of Earth's rotation, therefore this section is inactive.
         * If another gravity model such as EGM2008, whose potential function is described
         * without Earth's rotation, then this section should be activated.
         */
        res -= gravity_rotational();
      }
